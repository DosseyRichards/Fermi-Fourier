...
An owner-controlled gate with a mandatory delay between queueing and
executing proposals. Any caller can execute a proposal once its `eta`
passes (so an unavailable owner does not block upgrades); the owner
can also cancel before execution.
...
Reserves slots 0–10. Inheriting contracts must start their storage
at slot 11 or higher.
...
indefinite execution: if no caller invokes `execute` within `grace`
seconds after `eta`, the proposal silently expires. The next
`execute(id)` then fails the upper-bound check
`timestamp() < eta + grace`.
...
different values require source changes before compiling.
...
Proposals carry a **single 32-byte argument** because the contract
defines only one `p_arg` mapping. This covers most upgrade-flavored
calls:

- `transfer_ownership(addr)` — one argument.
- `set_scheme(id, addr)` — two arguments; requires packing both into
  one word (for example, `(id << 160) | addr`) or extending the
  contract to store a second arg mapping.
- Arbitrary multi-arg calls — extend the contract to store a
  `bytes`-hash commit/reveal scheme. Out of scope here.
...
1. The foundation operator deploys `CryptoRegistry` (init: owner =
   foundation operator).
2. The foundation operator deploys `Timelock` (init: owner =
   foundation operator, delay = 14d).
3. The foundation operator calls
   `registry.transfer_ownership(timelock_address)`.

After step 3:

- The registry's owner is the Timelock.
- The foundation operator queues `set_scheme(id, addr)` proposals in
  the Timelock.
- After the 14-day delay, the foundation operator (or any caller)
  invokes `timelock.execute(id)`, which calls
  `registry.set_scheme(id, addr)` with the Timelock as `caller()`.
...
The source comment reads "shortcut for v1; harden later." A
production-grade Timelock would require `caller() == address(this)`;
ownership transfer would itself need to be queued and timelocked.
This version trusts the owner to act correctly.

## Anybody-can-execute

After `eta`, **any** address can call `execute(id)`. This is by
design: queued proposals proceed even when the owner becomes
unavailable. The trade-off is the loss of the owner's veto window
after `eta`. For owner-only execution, gate `execute` with
`require(caller() == owner)`.
...
than inheriting it: the foundation operator deploys one Timelock and
points each governable contract at the same Timelock address.

Under inherit-by-copy, renumber all `@` slots to sit above the
consuming contract's own storage.
